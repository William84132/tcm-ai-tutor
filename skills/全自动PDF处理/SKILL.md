...
### 方法一：单文件处理
```
python "【外部参考·不随发布】 <PDF文件路径>
```

示例：
```bash
python "【外部参考·不随发布】 "【外部参考·不随发布】
```

### 方法二：批量处理目录
```bash
python "【外部参考·不随发布】 --batch "PDF文件夹路径"
```

示例：
```bash
python "【外部参考·不随发布】 --batch "【外部参考·不随发布】
```
...
## 配置文件路径
```python
TESSERACT_PATH = r"<Tesseract安装路径>\tesseract.exe"
TESSDATA_PATH = r"<Tessdata语言包目录>"
POPPLER_PATH = r"<Poppler解压目录>\poppler\poppler-26.02.0\Library\bin"
```
...
2. 解压到 `<Poppler解压目录>\poppler`
3. 确保 `<Poppler解压目录>\poppler\poppler-xx.xx\Library\bin` 存在
